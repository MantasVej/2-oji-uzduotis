...
void Studentai(list<Mokinys>& mas) {
    int n = 1;
    int paz;
    int s;
...
void Galutinis(list<Mokinys>& temp, char galutinis)
{
    for(auto&s : temp){
...
void Vidurkis(Mokinys& temp) {
    ...
    for (it = temp.v.begin(); it != temp.v.end(); it++) sum += *it;
...
void Mediana(Mokinys& temp) {
    ...
    for (it1 = temp.v.begin(); it1 != temp.v.end(); it1++)
        for (it2 = temp.v.begin(), 1; it2 != temp.v.end(); it2++) if (*it1 > *it2) std::swap(*it1, *it2);
    it1 = temp.v.begin();
    it2 = temp.v.begin();
    if ((n % 2) == 0) {
        std::advance(it1, n / 2);
        std::advance(it2, n / 2 - 1);
        mediana = (*it1 + *it2) / 2.0;
    }
    else {
        std::advance(it1, n / 2);
...
void Ivestis(Mokinys& temp) {
    ...
    bool check;
    int skaicius;
    //list<Mokinys>::iterator it = temp.v.begin();
...
void Isvedimas(list<Mokinys>& mas, string failas) {
    std::ofstream fr(failas);
    std::stringstream my_buffer;
    list<Mokinys>::iterator it = mas.begin();
    my_buffer << "Vardas              Pavarde             Galutinis" << endl;
    my_buffer << "-------------------------------------------------" << endl;
    for (it = mas.begin(); it != mas.end(); it++) {
        my_buffer << left << setw(20) << it->vardas << left << setw(20) << it->pavarde << std::fixed << std::setprecision(2) << left << setw(20) << it->galutinis << "\n";
    };
    fr << my_buffer.str();
    fr.close();
}
...
void Skaitymas(list<Mokinys>& mas, int& i, string failas) {
...
void Rusiavimas(list<Mokinys>& mas, int n)
{
    string failas1 = "vargsiukai" + std::to_string(n) + ".txt";
    string failas2 = "galvociai" + std::to_string(n) + ".txt";
    list<Mokinys> vargsiukai;
    list<Mokinys> galvociai;
    auto start = std::chrono::high_resolution_clock::now(); auto st=start;
    mas.sort(Palyginti);
    for(auto&s : mas){
        if (s.galutinis < 5) vargsiukai.push_back(s);
